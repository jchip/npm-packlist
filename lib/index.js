...
class PackWalker extends IgnoreWalker {
  ...
  onstat (opts, callback) {
    // Check for symlinks first before filtering by file/directory type
    // This allows includeSymlinks to work correctly
    if (opts.st.isSymbolicLink()) {
      if (!this.includeSymlinks) {
        return callback()
      }
      // Include symlinks when includeSymlinks is enabled
      // Continue to process them as files
